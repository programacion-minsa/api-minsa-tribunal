...
public class TribunalService : ITribunalService
{
    ...
    public TribunalService(IHttpClientFactory httpClientFactory, IHttpContextAccessor httpContextAccessor,
        IDbContextFactory<DBContext> dbContext, IImageService imageService)
    {
    ...
    public async Task<ResponseModel<FindPersonModel>> GetPatientById(string id, string apiKey)
    {
        bool allowDownloadImages = false;
        await using var localContext = await _dbContext.CreateDbContextAsync();
        var user = await localContext.AppsAccessToken.Where(x => x.AppToken == apiKey).FirstOrDefaultAsync();
        if (user != null)
        {
            allowDownloadImages = user.AllowGetImages ?? false;
        }

        var response = await _httpClient.GetAsync($"VerificarPersona?Cedula={id}");
        response.EnsureSuccessStatusCode();
        string xmlResponse = await response.Content.ReadAsStringAsync();
        var jsonResponse = TransformXmlToJson(xmlResponse, allowDownloadImages);
    ...
    public async Task<ResponseModel<bool>> StoreUserPetitionData(string id, bool responseStatus, string responseMessage)
    {
        ...
        string userAgent = headers != null && headers.ContainsKey("User-Agent")
    ...
    private ResponseModel<FindPersonModel> TransformXmlToJson(string xmlContent, bool allowDownloadImages)
    {
            ...
            if (innerXmlString != null)
            {
                ...
                if (personaPublica != null)
                {
                    ...
                    personaData.ApellidoPaterno =
                        ((string)personaPublica.Element(ns + "apellido_paterno")! ?? "").Trim();
                    personaData.ApellidoMeterno =
                        ((string)personaPublica.Element(ns + "apellido_materno")! ?? "").Trim();
                    ...
                    personaData.TipoSangre =
                        ((string)personaPublica.Element(ns + "descripcion_tipo_sangre")! ?? "").Trim();
                    ...
                    personaData.LugarNacimiento =
                        ((string)personaPublica.Element(ns + "LugarDeNacimiento")! ?? "").Trim();
                    ...
                    personaData.ResidenciaBarrio =
                        ((string)personaPublica.Element(ns + "barrio_residencia")! ?? "").Trim();
                    personaData.ResidenciaCalle =
                        ((string)personaPublica.Element(ns + "calle_residencia")! ?? "").Trim();
                ...
                if (allowDownloadImages)
                {
                    if (imagenes != null)
                    {
                        var cedulaImage = new CedulaImageModel() { };

                        string xmlCedulaImage = ((string)imagenes.Element(ns + "UrlFoto")! ?? "").Trim();
                        if (!string.IsNullOrEmpty(xmlCedulaImage))
                        {
                            var respuesta = Task
                                .Run(async () => await _imageService.DownloadToTempAsync(xmlCedulaImage))
                                .GetAwaiter().GetResult();
                            if (respuesta.Success)
                                cedulaImage.CedulaFoto = respuesta.Data ?? "";
                        }

                        string xmlCedulaFirma = ((string)imagenes.Element(ns + "UrlFirma")! ?? "").Trim();
                        if (!string.IsNullOrEmpty(xmlCedulaFirma))
                        {
                            var respuesta = Task
                                .Run(async () => await _imageService.DownloadToTempAsync(xmlCedulaFirma))
                                .GetAwaiter().GetResult();
                            if (respuesta.Success)
                                cedulaImage.CedulaFirma = respuesta.Data ?? "";
                        }

                        findData.CedulaImagenes = cedulaImage;
                    }
                }
    ...
    private string GetErrorMessage(int code, string message = "")
    {
        ...
        if (code == 534)
        {
            ...
            fechaDefuncion =
                $"{dateStringArray[0].PadLeft(2, '0')}-{monthCode.ToString().PadLeft(2, '0')}-{dateStringArray[2]}";
        }
